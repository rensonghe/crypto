...
path_signal='/home/xianglake/songhe/crypto_backtest/xrpusdt/%s_%s_20230201_0228_%sbar_%s_ST1.0_20230908_filter_%s_%s_99sec.csv' % (exchange, symbol,bar,p, out_threshold,amount)
    ...
    if net_position!=0:
        ...
        for l in range(len(deal_order_qty_series) - 1, -1, -1):
            ...
            if int(qtt) >= int(abs(net_position)):
                ...
                avg_deal_price =np.dot(deal_order_price_series[-1:-(cnt+1):-1],deal_order_qty_series[-1:-(cnt+1):-1])/sum(deal_order_qty_series[-1:-(cnt+1):-1])
                break
        # for l in range(len(deal_order_qty_series)-1,-1,-1):
        #     qtt=qtt+deal_order_qty_series[l]
        #
