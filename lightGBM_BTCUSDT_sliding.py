...
train_set_ = train_set_.iloc[:,5:93] #90
train_target_ = train_["target"]

train_set = train[train_col]
train_set = train_set.iloc[:,5:93] #90
train_target = train["target"]

test_set = test[train_col]
test_set = test_set.iloc[:,5:93]
...
def LGB_bayesian(learning_rate, num_leaves, bagging_fraction, feature_fraction, min_child_weight, min_child_samples,
    ...
    weight = [0.001, 0.004, 0.005, 0.02, 0.97]
    flag = 0
    for fold, (train_index, val_index) in enumerate(kf.split(X_train_, X_train_target_)):

        x_train, x_val = X_train_[flag:len(train_index)], X_train_[val_index]
        y_train, y_val = X_train_target_[flag:len(train_index)], X_train_target_[val_index]
        flag += 10000
...
def lightgbm_model(X_train, X_train_target,X_train_, X_train_target_, X_test, X_test_target, X_test_, X_test_target_, LGB_BO):
    ...
    features = train_.iloc[:, 5:93].columns
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X_train_, X_train_target_)):
        ...
        # y_train, y_val = X_train_target_[train_index], X_train_target_[val_index]
        # print('flag:',flag)
        # print('train_index:', len(train_index)+flag)
...
df_1 = predict.loc[predict['predict']>np.percentile(y_pred_train_[-50000:], 95)]
df_0 = predict.loc[predict['predict']<np.percentile(y_pred_train_[-50000:], 5)]
...
signal_1 = signal[signal['predict']>=np.percentile(y_pred_train_[-50000:], 95)]
signal_0 = signal[signal['predict']<=np.percentile(y_pred_train_[-50000:], 5)]
...
test_set = test_set.iloc[:,5:93]#65
...
def secondary_lightgbm_model(X_train, X_train_target, X_test, X_test_target, LGB_BO):
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X_train, X_train_target)):
        print('Model:',fold)
        x_train, x_val = X_train[flag: len(train_index)], X_train[val_index]
        y_train, y_val = X_train_target[flag: len(train_index)], X_train_target[val_index]
        flag += 20000
        # train_weight = [1 if i == 0 else 2 for i in y_train.tolist()]
        # test_weight = [1 if i == 0 else 2 for i in y_val.tolist()]
        train_set = lgb.Dataset(x_train, y_train)
...
#%%
print('long side threshold:',np.percentile(y_pred_train, 95))
print('short side threshold:',np.percentile(y_pred_train, 5))
print('out threshold:',np.percentile(secondary_y_pred_train, 80))
#%%
...
for out_threshold in out_threshold_set:
    ...
    final_df.to_csv(
        '/home/xianglake/songhe/crypto_backtest/{}/binance_{}_20230901_0930_{}bar_{}_ST1.2_20230930_filter_{}_{}.csv'.format(
