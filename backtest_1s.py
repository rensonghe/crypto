...
end_date = '2023-05-01'
path_signal='/home/xianglake/songhe/crypto_backtest/%s/%s_%s_20230401_0430_%sbar_%s_ST1.1_20230930_filter_%s_%s.csv' % (symbol,exchange, symbol,bar,p, out_threshold,amount)
    ...
    if match_signal is not None:
        ...
        if place_order_size_series != []:
            if abs(np.dot(place_order_size_series, place_order_price_series) + \
                   net_position * quote_row['close']) <= balance * max_margin:
                # 当仓位不超限时，下单
                place_order_price_series.append(place_order_price)
                place_order_qty_series.append(place_order_qty)
                place_order_side_series.append(place_order_side)
            ...
            if abs(net_position * quote_row['close']) < balance * max_margin:
                # 当仓位不超限时，下单
                place_order_price_series.append(place_order_price)
                place_order_qty_series.append(place_order_qty)
                place_order_side_series.append(place_order_side)
...
plt.show()
plt.savefig('/home/xianglake/yiliang/plot/%s_pnl_withstop_%s_%s_%s_%s_%s_%sp_%sl_sec.png'%(symbol, start_date, p,out_threshold,amount,slippage, pf_p,pf_l), dpi=750, bbox_inches = 'tight')
pnl_result.to_csv('/home/xianglake/yiliang/log/%s_pnl_withstop_%s_%s_%s_%s_%s_%sp_%sl_sec.csv'%(symbol, start_date, p,out_threshold,amount,slippage, pf_p,pf_l),
            index=True, sep=',')
...
trades_result.to_csv('/home/xianglake/yiliang/log/%s_trades_withstop_%s_%s_%s_%s_%s_%sp_%sl_sec.csv'%(symbol, start_date, p,out_threshold,amount,slippage, pf_p,pf_l),
            index=True, sep=',')