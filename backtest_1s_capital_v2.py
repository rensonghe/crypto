...
end_date = '2023-10-01'
path_signal='/home/xianglake/songhe/crypto_backtest/%s/%s_%s_20230901_0930_%sbar_%s_ST1.0_20231018_filter_90_%s_%s.csv' % (symbol,exchange, symbol,bar,p, out_threshold,amount)
...
for index, quote_row in quote_data.iterrows():
    ...
    if match_signal is not None:
        ...
        # 计算当前挂单价格与实时收盘价差距
        if abs(np.log(abs(order_price) / quote_row['close'])) >= 0.001:
            place_order_price = round(
                (quote_row['close'] + quote_row['close'] * place_order_side * slippage) * digit) / digit
        else:
            place_order_price = order_price
...
plt.show()
pnl_result.to_csv('/home/xianglake/yiliang/log/%s_pnl_withstop_%s_%s_%s_%s_%s_%sp_%sl_sec_capital_v2.csv' % (
symbol, start_date, p, out_threshold, amount, slippage, pf_p, pf_l),
                  index=True, sep=',')
trades_result = pd.concat(
    [pd.Series(deal_order_time_series), pd.Series(deal_order_size_series), pd.Series(deal_order_price_series)], axis=1)
trades_result.to_csv('/home/xianglake/yiliang/log/%s_trades_withstop_%s_%s_%s_%s_%s_%sp_%sl_sec_capital_v2.csv' % (
symbol, start_date, p, out_threshold, amount, slippage, pf_p, pf_l),
                     index=True, sep=',')