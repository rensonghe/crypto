...
def hf_trend_str(depth, trade, rolling=120):
    ...
    first_close = p.rolling(rolling).apply(lambda rows: rows[0], engine='numba', raw=True)
    last_close = p.rolling(rolling).apply(lambda rows: rows[-1], engine='numba', raw=True)
...
def mid_price(depth, trade, rolling=120):
    mid = (depth['ask_price1'] + depth['bid_price1'])/2
    return mid
