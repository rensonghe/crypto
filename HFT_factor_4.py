...
#%%  计算这一行基于bid和ask的wap
def calc_wap1(depth, trade):
    wap = (depth['bid_price1'] * depth['ask_size1'] + depth['ask_price1'] * depth['bid_size1']) / (
...
def calc_ofi(depth, trade, rolling):
    a = depth['bid_size1'] * np.where(depth['bid_price1'] - depth['bid_price1'].shift(rolling) >= 0, 1, 0)
    b = (depth['bid_size1'] - depth['bid_size1'].shift(rolling)) * np.where(depth['bid_price1'] - depth['bid_price1'].shift(rolling) <= 0, 1, 0)
    c = depth['ask_size1'] * np.where(depth['ask_price1'] - depth['ask_price1'].shift(rolling) <= 0, 1, 0)
    d = (depth['ask_size1'] - depth['ask_size1'].shift(rolling)) * np.where(depth['ask_price1'] - depth['ask_price1'].shift(rolling) >= 0, 1, 0)
...
def bp_rank_amount_ratio(depth, trade, rolling=100):

    a = trade['amount'].copy().fillna(method='ffill')
    rank = ((depth.bid_price1.rolling(rolling).rank()) / rolling * 2 - 1).fillna(0)
    high = pd.Series(rank, index=trade.index).rolling(rolling).quantile(0.9)
    low = pd.Series(rank, index=trade.index).rolling(rolling).quantile(0.1)
    high_amount = np.where(rank>=high, a-a.shift(rolling), 0)
    low_amount = np.where(rank <= low, a - a.shift(rolling), 0)
    high_low_diff = high_amount - low_amount
    ratio = pd.Series(high_amount, index=trade.index).rolling(rolling).sum()/a-a.shift(rolling)
    ratio_diff = pd.Series(high_low_diff, index=trade.index).rolling(rolling).sum()/a-a.shift(rolling)
    return ratio, ratio_diff

def ap_rank(depth, trade, rolling=100):
    return ((depth.ask_price1.rolling(rolling).rank()) / rolling * 2 - 1).fillna(0)


def ap_rank_amount_ratio(depth, trade, rolling=100):
    a = trade['amount'].copy().fillna(method='ffill')
    rank = ((depth.bid_price1.rolling(rolling).rank()) / rolling * 2 - 1).fillna(0)
    high = pd.Series(rank, index=trade.index).rolling(rolling).quantile(0.9)
    low = pd.Series(rank, index=trade.index).rolling(rolling).quantile(0.1)
...
def MTS(depth, trade, rolling=60):
    ...
    v = trade['volume'].fillna(method='ffill').copy()
    a = trade['amount'].fillna(method='ffill').copy()
    signal_amount = p * abs(v)
    MTS = pd.Series(signal_amount, index=trade.index).rolling(rolling).corr(a)
...
def reverse_factor_w(depth, trade, rolling=60):
    ...
    v = trade['volume'].fillna(method='ffill').copy()
    signal_amount = p * abs(v)
    log_return = abs(np.log(p/p.shift(1)))
    ...
    reverse_factor_w = pd.Series(M_high, index=trade.index).rolling(rolling).sum() - pd.Series(M_low, index=trade.index).rolling(rolling).sum()

    return reverse_factor_w

def SR(depth, trade, rolling=60):
...
def amount_std(depth, trade, rolling=60):

    p = trade['price'].fillna(method='ffill').copy()
    v = trade['volume'].fillna(method='ffill').copy()
    signal_amount = p * abs(v)
    criterion = pd.Series(signal_amount, index=trade.index).rolling(rolling).quantile(0.1)
    filter_amount = np.where(signal_amount <= criterion, signal_amount, 0)
    amount_std = pd.Series(filter_amount, index=trade.index).rolling(rolling).std()

    return amount_std.fillna(0)


def amount_skew(depth, trade, rolling=60):

    p = trade['price'].fillna(method='ffill').copy()
    v = trade['volume'].fillna(method='ffill').copy()
    signal_amount = p * abs(v)
    criterion = pd.Series(signal_amount, index=trade.index).rolling(rolling).quantile(0.5)
    filter_amount = np.where(signal_amount<=criterion, signal_amount, 0)
    amount_skew = pd.Series(filter_amount, index=trade.index).rolling(rolling).skew()

    return amount_skew.fillna(0)

def amount_kurt(depth, trade, rolling=60):
    ...
    criterion = pd.Series(signal_amount, index=trade.index).rolling(rolling).quantile(0.5)
    filter_amount = np.where(signal_amount<=criterion, signal_amount, 0)
    amount_kurt = pd.Series(filter_amount, index=trade.index).rolling(rolling).kurt()

    return amount_kurt.fillna(0)
...
def add_factor_process(depth, trade, min=40):
    ...
    df['ofi'] = calc_ofi(depth=depth, trade=None, rolling=60*min)
    df['bid_slope'], df['ask_slope'] = calc_slope(depth=depth, trade=None)
    df['sp'], df['sp2'] = calc_dispersion(depth=depth, trade=None)
    ...
    df['bp_rank_amount_ratio'], df['bp_rank_amount_ratio_diff'] = bp_rank_amount_ratio(depth=depth, trade=trade, rolling=60*min)
    df['ap_rank_amount_ratio'], df['ap_rank_amount_ratio_diff'] = ap_rank_amount_ratio(depth=depth, trade=trade, rolling=60*min)
    ...
    df['MTS'] = MTS(depth=None, trade=trade, rolling=60*min)
    df['MTE'] = MTE(depth=None, trade=trade, rolling=60*min)
    df['reverse_factor_w'] = reverse_factor_w(depth=None, trade=trade, rolling=60*min)
    df['SR'] = SR(depth=None, trade=trade, rolling=60*min)
    df['amount_std'] = amount_std(depth=None, trade=trade, rolling=60*min)
    df['amount_skew'] = amount_skew(depth=None, trade=trade, rolling=60*min)
    df['amount_kurt'] = amount_kurt(depth=None, trade=trade, rolling=60*min)
    df['wap1'] = calc_wap1(depth=depth, trade=None)
