...
end_date = '2023-08-01'
path_signal = '/home/xianglake/songhe/crypto_backtest/%s/%s_%s_20230701_0730_%sbar_%s_ST1.0_20231107_filter_95_%s_%s_%s.csv' % (
symbol, exchange, symbol, bar, p, out_threshold, amount, sammple_min)
...
plt.show()
pnl_result.to_csv('/home/xianglake/yiliang/log/%s_pnl_withstop_%s_%s_%s_%s_%s_%sp_%sl_%s_sec_capital.csv' % (
symbol, start_date, p, out_threshold, amount, slippage, pf_p, pf_l, sammple_min),
                  index=True, sep=',')
trades_result = pd.concat(
    [pd.Series(deal_order_time_series), pd.Series(deal_order_size_series), pd.Series(deal_order_price_series)], axis=1)
trades_result.to_csv('/home/xianglake/yiliang/log/%s_trades_withstop_%s_%s_%s_%s_%s_%sp_%sl_%s_sec_capital.csv' % (
symbol, start_date, p, out_threshold, amount, slippage, pf_p, pf_l, sammple_min),
                     index=True, sep=',')
