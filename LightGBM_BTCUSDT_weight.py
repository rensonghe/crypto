...
#%%
time_1 = '2023-01-01 00:00:00'
time_2 = '2023-01-30 23:59:59'
...
train_set = train[train_col]
train_set = train_set.iloc[:,5:105] #105
train_target = abs(train['target']).apply(lambda x:abs_classify(x, vol))
...
test_target = abs(signal_df["target"]).apply(lambda x:abs_classify(x, vol))
# test_target = abs(signal_df["volaility"]).apply(lambda x:abs_classify(x)) #realized volaility
print(signal_df['target'].apply(lambda x:abs_classify(x, vol)).value_counts())
# print(signal_df['volaility'].apply(lambda x:abs_classify(x)).value_counts())
print(len(signal_df[signal_df['target'].apply(lambda x:abs_classify(x, vol))==1])/len(signal_df['target'].apply(lambda x:abs_classify(x, vol))))
...
X_test_target = np.array(test_target)
#
from seondary_model_LightGBM import secondary_lightgbm_model, seondary_model_train_test
...
# print('70 out threshold:',np.percentile(secondary_y_pred_train, 70))
...
for out_threshold in out_threshold_set:
    ...
    final_df.to_csv(
        '/home/xianglake/songhe/crypto_backtest/{}/binance_{}_20230101_0130_{}bar_{}_ST1.0_20231110_filter_95_{}_{}.csv'.format(
